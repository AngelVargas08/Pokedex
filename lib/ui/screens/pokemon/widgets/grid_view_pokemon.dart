...
class _GridViewPokemonState extends State<GridViewPokemon> {
      ...
      child: Consumer<PokemonProvider>(
        builder: (context, value, __) {
          return GridView.builder(
            ...
            itemCount: value.isloadinPagination
                ? value.onDisplayPokemon.length + 1
                : value.onDisplayPokemon.length,
            itemBuilder: (context, index) {
              if (index >= value.onDisplayPokemon.length) {
                return Padding(
              ...
              final pokemon = value.onDisplayPokemon[index];
  ...
  void _onScrollUpdade() {
    var maxScroll = _controller.position.maxScrollExtent;
    var currentPosition = _controller.position.pixels;

    if (currentPosition > maxScroll - 150) {
      var state = context.read<PokemonProvider>();
      state.loadNextpage();
    }
  }
}
